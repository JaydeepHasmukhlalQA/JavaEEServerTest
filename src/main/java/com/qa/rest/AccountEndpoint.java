...
//	}
	...
	@Path("/accounts/{name}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getOneName(@PathParam("name") String name) {
		if (accountRepository.readByName(name).equals(null)) {
			return Response.status(Status.NOT_FOUND).build();
		}
		Account account = accountRepository.readByName(name);
		return Response.ok(account).build(); 
	}
